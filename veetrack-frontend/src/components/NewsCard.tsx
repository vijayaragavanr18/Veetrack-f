...
interface NewsCardProps {
  article: Article;
  stateClass: 'active' | 'prev' | 'next' | '';
  onReadFullStory: (article: Article) => void;
...
export const NewsCard: React.FC<NewsCardProps> = ({
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.answer) {
          setChatMessages([...updatedMessages, { role: 'assistant' as const, text: data.answer }]);
        }
      } else {
        setChatMessages([
          ...updatedMessages,
          { role: 'assistant' as const, text: "Sorry, I couldn't reach the server. Please try again." },
        ]);
      }
    } catch (error) {
      ...
      setChatMessages([
        ...updatedMessages,
        { role: 'assistant' as const, text: 'An error occurred while answering your question.' },
      ]);
    } finally {
      ...
      <div className="flex-1 overflow-hidden relative flex flex-col pt-1 pb-8">
              ...
              <div className="flex-1 overflow-y-auto space-y-4 pr-1 scrollbar-thin">
                <div>
                  ...
                  <ul className="space-y-2 font-body-lg text-body-md md:text-body-lg text-on-surface-variant leading-relaxed">
                    {article.whatHappened?.map((point, index) => (
                    ...
                    )) || <li className="text-on-surface-variant/40 italic">No details available.</li>}
                  </ul>
                </div>
                <div>
                  <h4 className="text-[10px] font-bold text-amber-400 uppercase tracking-wider mb-2 mt-2">Why It Happened</h4>
                  <ul className="space-y-2 font-body-lg text-body-md md:text-body-lg text-on-surface-variant leading-relaxed">
                    {article.whyItMatters?.map((point, index) => (
                      <li key={index} className="flex gap-2 items-start text-left">
        ...
        <div className={`absolute bottom-8 left-container-padding right-container-padding z-30 select-none ${isTop ? 'pointer-events-none' : ''}`}>
          <div 
            ...
            onTouchStart={(e) => e.stopPropagation()}
            tabIndex={isTop ? -1 : 0}
            className={`flex items-center gap-2 px-3 py-1.5 bg-surface-container-high border border-outline-variant/30 hover:border-primary-container/40 rounded-lg text-on-surface-variant/50 text-[10px] transition-all ${
          ...
          {[0, 1, 2, 3].map((idx) => (
            <button
              ...
              className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${
                ...
                idx === activePageIndex
                  ? 'bg-primary-container w-3.5'
                  : 'bg-on-surface-variant/20 hover:bg-on-surface-variant/40'
              }`}
              aria-label={`Go to sub-page ${idx + 1}`}
            />
          ))}
        </div>
        ...
        <div 
          ...
          className={`absolute inset-0 bg-surface-container-lowest border-t border-outline-variant/30 flex flex-col z-40 transition-transform duration-300 ease-out select-none ${
        ...
        >
          ...
          <div className="h-10 border-b border-outline-variant/20 px-container-padding flex justify-between items-center bg-surface-container-low shrink-0 select-none">
            ...
            <button
              ...
              tabIndex={isTop ? 0 : -1}
              disabled={!isTop}
              className={`text-on-surface-variant/60 hover:text-primary p-1 rounded-full transition-colors ${
                isTop ? 'cursor-pointer hover:bg-surface-container-high' : 'pointer-events-none opacity-40'
              }`}
          ...
          <div 
            ref={chatScrollRef}
            className="flex-1 overflow-y-auto p-4 space-y-3 scrollbar-thin select-text flex flex-col"
          >
            ...
            {chatMessages.map((msg, idx) => (
              <div 
                key={idx}
                className={`max-w-[85%] rounded-xl px-3.5 py-2 text-[11px] leading-relaxed select-text ${
                  msg.role === 'user'
                    ? 'self-end bg-primary-container/10 border border-primary-container/30 text-on-surface rounded-tr-none ml-auto'
                    : 'self-start bg-surface-container-high text-on-surface-variant rounded-tl-none mr-auto'
                }`}
              >
                {msg.text}
              </div>
            ...
            {isChatLoading && (
              <div className="max-w-[85%] self-start bg-surface-container-high text-on-surface-variant/40 rounded-xl rounded-tl-none px-3.5 py-2 text-[11px] flex items-center gap-1.5 mr-auto animate-pulse select-none">
          ...
          {/* Chat Input Form */}
          <form 
            onSubmit={!isTop ? handleSendMessage : (e) => e.preventDefault()}
            className="p-2 border-t border-outline-variant/20 bg-surface-container-low flex items-center gap-2 shrink-0 select-none"
          >
            <input
              type="text"
              placeholder="Ask a question..."
              value={chatInput}
              onChange={!isTop ? (e) => setChatInput(e.target.value) : undefined}
              onKeyDown={(e) => {
                e.stopPropagation(); // Stop arrow navigation while typing!
              }}
              tabIndex={isTop ? -1 : 0}
              readOnly={isTop}
              className="flex-1 bg-surface-container-highest border border-outline-variant/30 focus:border-primary-container rounded-lg px-3 py-1.5 text-[11px] text-on-surface placeholder:text-on-surface-variant/30 focus:outline-none transition-all"
            />
            <button
              type="submit"
              disabled={isTop || !chatInput.trim() || isChatLoading}
              tabIndex={isTop ? -1 : 0}
              className={`p-1.5 bg-primary-container text-on-primary-container rounded-lg transition-all flex items-center justify-center shrink-0 ${
                isTop ? 'pointer-events-none opacity-40' : 'hover:bg-primary-fixed-dim cursor-pointer'
              }`}
  ...
  return (
    <div className={`news-card-wrapper ${stateClass}`} data-index={article.id}>
