  ...
  return (
    <div className="w-full h-full flex items-center justify-center relative">
        ...
        {articles.map((_, idx) => (
          <button
            ...
            className={`w-2.5 h-2.5 rounded-full transition-all duration-300 cursor-pointer ${
              idx === currentIndex
                ? 'bg-primary-container scale-125 shadow-md shadow-primary-container/20'
                : 'bg-on-surface-variant/30 hover:bg-on-surface-variant/60'
            }`}
          />
        ))}
      </div>
      ...
      <div
        ...
        className={`perspective-container w-full max-w-[450px] sm:max-w-[540px] md:max-w-[620px] lg:max-w-[680px] h-full relative ${getContainerClass()}`}
        id="reader-container"
        style={{ touchAction: 'none' }}
      >
        {articles.map((article, idx) => {
          let stateClass: 'active' | 'prev' | 'next' | '' = '';
          ...
          return (
            <NewsCard
              ...
              activePageIndex={idx === currentIndex ? activeSubPageIndex : 0}
              onPageChange={idx === currentIndex ? setActiveSubPageIndex : undefined}
              onReadFullStory={onReadFullStory}
              dragOffset={idx === currentIndex ? dragOffset : 0}
              isDragging={idx === currentIndex ? isDraggingState : false}
            />
          );
        })}
      </div>
